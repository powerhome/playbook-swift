...
public struct PopoverCatalog: View {
    ...
    @State var clickLocation: CGPoint = .zero
    ...
    private var onClosePopover: some View {
        VStack(spacing: Spacing.medium) {
            ...
            PBButton(
                variant: .secondary,
                title: "Position popover!"
            ) {
                isPresented8.toggle()
            }
            .simultaneousGesture(DragGesture(minimumDistance: 0, coordinateSpace: .global)
                .onChanged {
                    clickLocation = $0.location
                    update.toggle()
                }
            )
            .pbPopover(
                isPresented: $isPresented8,
                id: 8,
                position: .custom(clickLocation),
                clickToClose: (.anywhere, action: {}),
                refreshView: $update
            ) {
                Text("Position popover!")
                    .pbFont(.body, color: .text(.default))
            }
        }
    }
