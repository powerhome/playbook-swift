...
function addBuildKeys {
  ...
  security import "$ROOT_DIR/fastlane/git_prov_profiles/certificates/ios_distribution.p12" -k "$KEYCHAIN_NAME" -P "$KEY_PASSWORD" -T /usr/bin/codesign
  ...
  security import "$ROOT_DIR/fastlane/git_prov_profiles/certificates/dev_id_application.p12" -k "$KEYCHAIN_NAME" -P "$KEY_PASSWORD" -T /usr/bin/codesign
