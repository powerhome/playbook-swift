...
final class PBCardTests: XCTestCase {
  ...
  func test_card_highlight_colors() throws {
    let cardAllowedColor = PBCard(highlightColor: .product(.windows)) { Text("test right color") }
    let cardWrongColor = PBCard(highlightColor: .active) { Text("test wrong color") }
    ...
    XCTAssertEqual(colorAllowed, .product(.windows))
    XCTAssertEqual(colorWrong, .active)
